# --------------------------------------------------------------------------------------------
# Copyright (c) Microsoft Corporation. All rights reserved.
# Licensed under the MIT License. See License.txt in the project root for license information.
#
# Code generated by aaz-dev-tools
# --------------------------------------------------------------------------------------------

from azure.cli.core import AzCommandsLoader
from azure.cli.core.commands import CliCommandType
from azure.cli.command_modules.computefleet._help import helps  # pylint: disable=unused-import
#from azure.cli.command_modules.computefleet.commands import computefleet_custom  # import computefleet_custom

class ComputefleetCommandsLoader(AzCommandsLoader):

    def __init__(self, cli_ctx=None):
        from azure.cli.core.profiles import ResourceType  # required when using python sdk
        #comoutefleet_custom = CliCommandType(operations_tmpl='azure.cli.command_modules.computefleet.custom#{}')
        super(ComputefleetCommandsLoader, self).__init__(cli_ctx=cli_ctx,
                                                         #custom_command_type=comoutefleet_custom,
                                                         resource_type=ResourceType.MGMT_COMPUTE)    # required when using python sdk

    def load_command_table(self, args):
        from .commands import load_command_table
        load_command_table(self, args)
        return self.command_table

    def load_arguments(self, command):
        from azure.cli.command_modules.computefleet._params import load_arguments
        load_arguments(self, command)

COMMAND_LOADER_CLS = ComputefleetCommandsLoader
